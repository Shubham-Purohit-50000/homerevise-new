                        ...
                        <form action="{{route('questions.update', ['question'=>$question->id])}}" method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <select name="question_type" id="question_type" class="form-control">
                                    ...
                                    <option value="{{$item['code']}}" {{$question->question_type == $item['code'] ? 'selected' : ''}}>{{$item['name']}}</option>
                                    @endforeach 
                                </select>
                                @error('question_type')
                                ...
                                <div class="col-sm-6">
                                    <div class="form-group">
                                        ...
                                        <select name="standards" id="standards" class="form-control">
                                            ...
                                            <option value="{{$item['id']}}" {{$question->standard_id == $item['id'] ? 'selected' : ''}}>{{$item['name']}}</option>
                                            @endforeach 
                                        </select>
                                        @error('standards')
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="question" id="question">{!! $question->questions!!}</textarea>
                                @error('question')
                            ...
                            <div id="optioncontainer" class="row">
                                ...
                                <div class="option-container col-sm-6">
                                    <label>Option {{$key}}</label>
                                    <div class="option-inner-container">
                                        <textarea  name="options[{{$key}}]" class="form-control summernote-textarea" required="true">{{$value}}</textarea>
                                        <button class="remove-button btn btn-danger text-white ml-2 my-2 mb-4">
                            ...
                            <div class="form-group" id="correct_answere">
                                ...
                                <select id="optionDropdown" name="correct_ans" class="form-control" >
                                @foreach(json_decode($question->options,true) as $key => $value)
                                    <option value="{{$key}}" {{$question->correct_answer == $key ? 'selected' : ''}}>Option {{$key}}</option>
                                @endforeach
                                </select>
                            </div>
                            <div class="form-group" id="correct_answere">
                                <label for="correct_ans">Correct Marks</label>
                                <input type="text" name="correct_marks" value="{{$question->correct_marks}}" class="form-control">
                                @error('correct_marks')
...
<script>
    $(document).ready(function() {                
        ...
        function addOption(status) {
            ...
            $('#addButton').show();
            optionCount++;
            // console.log(optionCount,"charCodeOfAcharCodeOfAcharCodeOfAcharCodeOfA"); 

            // Find the next available label (A, B, C, ...)
            let nextLabel = '';
            for (let i = 0; i < 26; i++) {
                const label = String.fromCharCode(optionCount + i);
                console.log(label,optionCount,"lablllllllllllllllllll");
            ...
            removeButton.className = 'remove-button btn btn-danger text-white ml-2 my-2 mb-4';
        ...
        getSubject({{$question->standard_id}});
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.data.forEach(function(subject) {
                            ...
                            console.log(subject.id,{{$question->subject_id}},"optionsssssssssss");
        ...
        getChapter({{$question->subject_id}});
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.data.forEach(function(subject) {
                            ...
                            option.val(subject.id);
                            option.text(subject.name);
                            if(subject.id == {{$question->chapter_id}}) {
                                option.prop('selected', true);
                            }
                            // Append the option to the select element
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        data.data.forEach(function(subject) {
                            ...
                            if(subject.id == {{$question->topic_id ? $question->topic_id : 0}}) {
                                option.prop('selected', true);
                            }
                            // Append the option to the select element
                            topic.append(option);
